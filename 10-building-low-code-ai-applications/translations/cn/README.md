...
1. 导航到 [Power Apps](https://make.powerapps.com?wt.mc_id=studentamb_409460-koreyst) 主屏幕。

1. 使用主屏幕上的文本区域描述您要构建的应用程序。 例如，**_我想构建一个应用程序来跟踪和管理学生作业_**。 单击 **发送** 按钮将提示发送到 AI Copilot。

![描述您要构建的应用程序](../../images/copilot-chat-prompt-powerapps.png?wt.mc_id=studentamb_409460-koreyst)
...
1. AI Copilot 将建议一个 Dataverse 表，其中包含存储要跟踪的数据所需的字段以及一些示例数据。 然后，您可以通过对话步骤使用 AI Copilot 助手功能自定义表格以满足您的需求。
   ...
   ![新表中的建议字段](../../images/copilot-dataverse-table-powerapps.png?wt.mc_id=studentamb_409460-koreyst)
...
![添加新字段](../../images/copilot-new-column.png?wt.mc_id=studentamb_409460-koreyst)
...
![Describe the app you want to build](../../images/copilot-new-screen.png?wt.mc_id=studentamb_409460-koreyst)
...
![选择新表](../../images/describe-new-table.png?wt.mc_id=studentamb_409460-koreyst)

1. 在 **描述新表** 屏幕上，使用文本区域描述您要创建的表。 例如，**_我想创建一个表来存储发票信息_**。 单击 **发送** 按钮将提示发送到 AI Copilot。

![描述表格](../../images/copilot-chat-prompt-dataverse.png?wt.mc_id=studentamb_409460-koreyst)

1. AI Copilot 将建议一个 Dataverse 表，其中包含存储要跟踪的数据所需的字段以及一些示例数据。 然后，您可以通过对话步骤使用 AI Copilot 助手功能自定义表格以满足您的需求。

![建议的 Dataverse 表](../../images/copilot-dataverse-table.png?wt.mc_id=studentamb_409460-koreyst)
...
通过自定义 AI 模型，您可以将自己的模型引入 AI Builder，以便它可以像任何 AI Builder 自定义模型一样运行，从而允许您使用自己的数据训练模型。 您可以使用这些模型来自动化流程并预测 Power Apps 和 Power Automate 中的结果。 使用您自己的模型时存在一些限制。 了解有关这些[限制](https://learn.microsoft.com/ai-builder/byo-model#limitations?wt.mc_id=studentamb_409460-koreyst)的更多信息

![AI 构建器模型](../../images/ai-builder-models.png?wt.mc_id=studentamb_409460-koreyst)
...
1. 导航至 [Power Automate](https://make.powerautomate.com?wt.mc_id=studentamb_409460-koreyst) 主屏幕。

2. 使用主屏幕上的文本区域描述您要构建的工作流程。 例如，**_当发票到达我的邮箱时对其进行处理_**。 单击 **发送** 按钮将提示发送到 AI Copilot。

   ![副驾驶电源自动化](../../images/copilot-chat-prompt-powerautomate.png?wt.mc_id=studentamb_409460-koreyst)
...
8. 从流程中删除 **Condition** 操作，因为您不会使用它。 它应该类似于以下屏幕截图：
   ...
   ![电源自动化，删除操作](../../images/powerautomate-remove-actions.png?wt.mc_id=studentamb_409460-koreyst)
...
11. 选择您之前创建的 **表**。 例如，您可以选择 **发票信息** 表。 从上一个操作中选择动态内容以填充以下字段：
    ...
    ![电源自动添加行](../../images/powerautomate-add-row.png?wt.mc_id=studentamb_409460-koreyst)
...
![创建提示](../../images/create-prompt-gpt.png?wt.mc_id=studentamb_409460-koreyst)
...
想要了解有关创建低代码的人工智能应用的更多信息？ 转至[进阶学习的页面](https://aka.ms/genai-collection?wt.mc_id=studentamb_409460-koreyst) 查找有关此主章节的其他学习资源。

前往第十一章，我们将学习[为生成式 AI 添加 function calling](../../../11-integrating-with-function-calling/translations/cn/README.md?wt.mc_id=studentamb_409460-koreyst)
